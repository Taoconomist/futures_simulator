...
with st.sidebar.expander("策略收益参数"):
    ...
    # 扩大期权权利金范围至0-150元
    option_premium = st.slider("期权权利金（元/吨）", 0, 150, 20)
...
# 计算年化收益率
def calculate_annualized_return(total_profit, capital, risk_free_rate, years=1):
    """
    计算年化收益率和超额收益率
    """
    # 总收益率 = 总利润 / 总资金
    ...
    annualized_return = ((1 + total_return) ** (1/years)) - 1 if total_return > -1 else 0
...
with tab2:
    ...
    annualized_return, excess_return = calculate_annualized_return(
        max_profit, capital, risk_free_rate
    )
...
with tab3:
    ...
    try:
        ...
        st.warning("流程图功能需要安装Graphviz。请参考安装说明。")
    ...
    with st.expander("2. 网格增强策略"):
        ...
        <p><b>风险管理</b>:
            <ul>
                <li>网格密度优化：根据市场波动率调整网格间距</li>
                <li>滑点控制：设置最大可接受滑点</li>
                <li>交易频率限制：防止高频交易导致成本上升</li>
            </ul>
        </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.expander("3. 期权卖权策略 - 增强版"):
        ...
        <p><b>操作</b>:
            <ul>
                ...
                <li>权利金范围扩大至0-150元/吨，增加策略灵活性</li>
            </ul>
        </p>
        <p><b>风险管理</b>:
            <ul>
                ...
                <li><b>压力测试</b>：模拟黑天鹅事件下的期权损失</li>
                <li><b>保证金监控</b>：实时监控期权保证金占用</li>
            </ul>
        </p>
        </div>
        """, unsafe_allow_html=True)
...
with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
    ...
    # 风险指标表
    risk_metrics = {
        "指标": ["最大回撤", "盈亏平衡区间", "95% VaR", "压力测试亏损", "年化收益率", "超额收益率"],
        "数值": [
